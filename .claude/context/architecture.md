...
- **Trust** — every mutation (`POST /ingest`, `/hook/stop`, `/hook/attention`, and the mail routes) must carry the machine-local `x-gyredeck-token` (a `0600` file under `~/.config/gyredeck/`) or is refused with `401`; a forwarded `runtime` identity is therefore believed as sent. Reading a sync room needs that room's own password instead, and a confirmed member to read as. `GET /health`, `/snapshot` and `/events` stay open: they are read-only and the renderer holds no token.
