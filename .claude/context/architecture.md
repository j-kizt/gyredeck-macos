...
- **Mail rooms** — `POST /mail/<room>` plus `GET /mail/<room>/events` (subscribe) or `GET /mail/<room>?since=<seq>` (read the backlog) let agents on this machine message each other over the bridge port instead of a second listener. A message carries `to` and `kind`: `to` routes it, and `kind` says what may be said back — an `ask` is answered, a `tell` or a `notice` may draw a `reaction`, and nothing answers a reaction. Token-required, in-memory, never written to the event log. See [`event-protocol.md`](event-protocol.md).
- **Audit log** — appends every event as newline-delimited JSON to `~/.config/gyredeck/gyredeck.events.ndjson` (local diagnostics, not telemetry). Every line carries a `conversationId`, `cwd`, `model` and `permissionMode`, so it is a record of what each agent was doing and where. The bridge opens it once with `O_NOFOLLOW` and sets `0600` through that descriptor before writing — narrowing a log an older bridge left at `0644` — and if it cannot confirm a private regular file it **turns disk logging off** and says so on stderr rather than appending anyway. A log it gave up on keeps the mode it had. It is also **bounded**: past 8 MiB the log is moved aside to `gyredeck.events.ndjson.1` — at startup and again while running, since the bridge stays up for days — and one generation is kept. Rotation happens only once the descriptor has been checked and narrowed, never by path beforehand: reaching for the name first would let a symlink be renamed and its target chmodded, which is what `O_NOFOLLOW` is there to refuse. Every open and reopen goes through the same check, so a file that appears at the path mid-run earns its own. Hydration reads the rotated generation before the live log, because the event that carries the log over the cap lands in the file that is then moved aside. The pair is *about* twice the cap rather than exactly: `/ingest` puts no ceiling on a single event, so one oversized record can carry a generation past it.
- **Trust** — `POST /ingest`, `/hook/stop` and `/hook/attention` must carry the machine-local `x-gyredeck-token` (a `0600` file under `~/.config/gyredeck/`) or are refused with `401`, so a forwarded `runtime` identity is believed as sent. The mail routes take a credential in that same header, but not always the same one: reading a sync room needs that room's own password and a confirmed member to read as, while sending accepts the room's password or the machine token — a founder holds no password until somebody reads one out. `GET /health`, `/snapshot` and `/events` stay open: they are read-only and the renderer holds no token.
- **Origin** — a request carrying a browser `Origin` is answered only for the packaged webview (`tauri://localhost` on macOS) or the dev server the same renderer runs from (`http://127.0.0.1:47622`, `vite.config.ts`'s `strictPort`); anything else gets `403 forbidden_origin` before the route runs, and is named once in the bridge's stderr. A request with no `Origin` is a process, not a page — every adapter and the whole native side — and is served as before.
