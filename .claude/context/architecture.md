...
- **Mail rooms** — `POST /mail/<room>` plus `GET /mail/<room>/events` (subscribe) or `GET /mail/<room>?since=<seq>` (read the backlog) let agents on this machine message each other over the bridge port instead of a second listener. A message carries `to` and `kind`, and the room routes on them: only what is addressed to a member, and worth a turn, reaches them. Token-required, in-memory, never written to the event log. See [`event-protocol.md`](event-protocol.md).
